...
function listOfDates(first_date, last_date) {
  ...
  while (date <= date2) {
    //while the date is not the second date, increment the date and push it to the array of dates
...
input_form.addEventListener("submit", function submitFunction(e) {
  e.preventDefault();
  form.list_of_dates = listOfDates(
    //create the list of dates from the input
  ...
  $.ajax({
    //pushes the results to the page without needing to refresh the page
    url:
      "db.php?first_date=" +
      form.first_date.value +
      "&last_date=" +
      ...
      form.catering_grade +
      "&list_of_dates=" +
      form.list_of_dates, //pass the variables into the db.php
    type: "POST",
    data: json_form,
...
function makeCard(data) {
  ...
  for (const [day, venues] of Object.entries(data)) {
    //seperates the key(the date) from the venues and loops through the venues
    ...
    venues.forEach((day_result) => {
      //loops through the venue to find the data to put onto the card
    ...
    <img class="card-img-top" src="/images/${name}.jpg" alt="Card image cap">
    <div class="card-body">
